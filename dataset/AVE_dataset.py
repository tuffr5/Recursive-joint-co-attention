...
class AVEDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # 10*1
        # event_cov = np.zeros(10)
        # for i in range(10):
        #     event_cov[i] = 0 if np.argmax(label[i, :]) == 28 else 1

        # return {'audio': torch.from_numpy(audio).float(), 'video': torch.from_numpy(video).float(),
        #         'label': torch.from_numpy(label).float(), 'event_label': torch.from_numpy(event_cov).float()}
        return {'audio': torch.from_numpy(audio).float(), 'video': torch.from_numpy(video).float(),
